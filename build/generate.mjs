...
const manifest = {
  ...
  install: {
    oneCommand: `curl -fsSL ${SITE}/install.sh | sh`,
    oneCommandOptIn: "append `-s -- --with-ghost` for a self that persists across sessions, `--guard` to stop agents printing secret files",
    note: "Zero dependencies. Node 22+ (needs built-in node:sqlite). The one command clones, links and registers everything; or clone and register by hand:",
...
Install everything with one command: \`curl -fsSL ${SITE}/install.sh | sh\` — it clones the tools into ~/.tools-for-agents, links their CLIs and registers their MCP servers with Claude Code (append \`-s -- --with-ghost\` for a self that persists, \`--guard\` to stop agents printing secret files).
